...
## 改动内容
1. 废除了所有源码中的 `root` 及空密码默认值。引入了强制的环境变量 (`DORIS_PASSWORD` 等) 检查。
2. 新增了 `ops/bin` 标准化控制脚本，带有操作日志 (`/var/log/...`)、`--confirm` 二次确认、`DRY_RUN` 测试以及禁止 Job 重复提交的保护。
3. 提供了 `ops/checks` 供自动化运维 (如 DataSentry) 安全提取线上事实指标 (只读方式)。
